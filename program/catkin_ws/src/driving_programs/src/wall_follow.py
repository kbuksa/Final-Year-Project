...
class Wall_follower:
    ...
    prev_error = 0.0
    prev_t = rospy.get_rostime() #previous time - init as current time
    ...
    def scan_listener(self, scan_data):
        ...
        index_a = math.floor((angle_a - scan_data.angle_min) / scan_data.angle_increment)
        index_b = math.floor((angle_b - scan_data.angle_min) / scan_data.angle_increment)
        ...
        angle_alpha = math.atan((range_a * math.cos(angle_a) - range.b) / range_a * math.sin(angle_a))
    ...
    def pid_control(self, dist_t1,):
        ...
        moment_now = rospy.get_rostime() #getting new current time reading 
